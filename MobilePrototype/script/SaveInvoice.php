<?php
	...
	if(isset($_POST["P_Code"]) && isset($_POST["Quantity"]) && isset($_POST["Total_Price"])
		&& isset($_POST["C_ID"]) && isset($_POST["INV_Date"]) && isset($_POST["UserID"])){	
		//$INV_ID = $_POST["INV_ID"];
		$P_Code = $_POST["P_Code"];
	...
	$statement = mysqli_prepare($con, "INSERT INTO Invoice (P_Code, Quantity, Total_Price, C_ID, INV_Date, UserID, Inv_Type) 
									   VALUES (?, ?, ?, ?, ?, ?, ?) ");
	mysqli_stmt_bind_param($statement, "ssssss", $P_Code, $Quantity, $Total_Price, $C_ID, $INV_Date, $UserID, $Inv_Type);
